...
export const site = {
  ...
  tec: {
    ...
    triple: {
      ...
      subtitle: 'Select a condition, then step through a possible parser repair.',
      ...
      details: [
        'Return an empty result for empty input, while preserving behavior on non-empty input. The request supplies the direction and scope of the repair.',
        'A working repository supplies files and test responses. Read, edit and test access defines how this run can interact with it.',
        'A model with instructions chooses actions from observations and history; file and test tools make those actions available.',
      ],
      runtime: 'Agent: the ongoing execution',
      stages: ['Observe', 'Choose', 'Act', 'Feedback'],
      trajectory: [
        'The agent reads the parser. Empty input currently reaches a branch that raises an error.',
        'Using the request and source code, it chooses an edit intended to handle empty input.',
        'It applies the edit to the working copy and runs tests. The environment has now changed.',
        'A regression test fails on non-empty input. The agent uses this observation to choose another edit and repeat the loop.',
      ],
    ...
    operators: {
      ...
      details: [
        ...
        'Assess the patch using empty-input and regression tests. The working copy was already changed by execution; passing tests supplies evidence for acceptance, not a proof for every input.',
