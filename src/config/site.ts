...
export const site = {
  ...
  tec: {
    ...
    triple: {
      ...
      subtitle: 'Select a condition, then step through a possible proof attempt.',
      tabs: ['Task', 'Environment', 'Capacity'],
      summaries: [
        'Requested outcome + scope',
        'State + interaction access',
        'Action selection + available actions',
      ],
      details: [
        'Prove that for every integer n, an even n² implies an even n. The integer domain and universal quantifier define the scope.',
        'The workspace holds definitions, the proof draft and review feedback. Access rules determine what this run can read, write and submit.',
        'A model with instructions chooses proof steps from observations and history; workspace and review interfaces let it record and assess arguments.',
      ],
      runtime: 'Agent: the ongoing execution',
      stages: ['Observe', 'Choose', 'Act', 'Feedback'],
      trajectory: [
        'The agent reads the definitions: even integers are 2k; odd integers are 2k + 1, for integer k.',
        'It tries a direct argument: n² = 2k, therefore n is even. The implication still needs justification.',
        'It writes the proposed argument into the proof workspace and requests review.',
        'Review identifies the unjustified implication. The agent switches to the contrapositive: n = 2m + 1 gives n² = 2(2m² + 2m) + 1, so odd n has an odd square.',
      ],
      continuation: 'Repeat the loop',
      stable: 'The setup can stay fixed while the environment, history and next action change.',
    },
    operators: {
      title: 'Five operators organize execution.',
      subtitle: 'An illustrative workflow: action and feedback repeat within it.',
      ...
      details: [
        'Create a task: prove that an integer with an even square is even. Require an argument covering all integers.',
        'Bind the task to a reasoning capacity with access to definitions, a proof workspace and review.',
        'Select the theorem, parity definitions, current draft and relevant review feedback for this model call.',
        'Review each inference and the coverage of all integers. Numerical examples alone cannot establish the theorem. The draft was already written before it was assessed.',
      ...
      loop: 'During execution: observe → choose → act → feedback → repeat. Assembly and checking can recur; these operators are not an exhaustive list of state changes.',
    ...
    topology: {
      ...
      subtitle: 'Each node is an execution unit. Each arrow means the next unit needs its output.',
      presets: ['Sequential review', 'Parallel review', 'Specialist review'],
      primitiveLabels: [
        'Tasks & dependencies',
        'Environment access',
        'Capacity & operator authority',
      ],
      setups: [
        [
          'Draft → Check A → Check B → Finalize',
          'Check B receives the draft with Check A’s findings; the finalizer receives both reviews.',
          'Reviewers use the same instructions; the finalizer assesses the result.',
        ],
        [
          'Both checks need the draft; finalization needs both reviews.',
          'Each reviewer receives the draft independently; neither reads the other’s findings.',
          'Reviewers use the same instructions; the finalizer assesses the result.',
        ],
        [
          'The same dependencies as parallel review.',
          'Check A receives the expansion; Check B receives the implication and integer assumptions.',
          'Check A specializes in algebra, Check B in logic; the finalizer assesses the result.',
        ],
      ],
      nodes: ['Draft', 'Check A', 'Check B', 'Finalize'],
      captions: [
        'Sequential review lets Check B use Check A’s findings, but it must wait for them. This changes information and scheduling together.',
        'Two reviewers assess the draft independently. Both must finish before finalization can use their findings.',
        'The four arrows stay unchanged. Specialized instructions and selected evidence change what the reviewers compute, even though the graph is identical.',
      ],
      edge: 'Arrow: requires the sender’s output',
      shared: 'Same reviewer instructions',
