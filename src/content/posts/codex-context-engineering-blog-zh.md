---
title: 'Codex 源码讲解（六）：Context Engineering'
description: 'AGENTS.md、Skills 与 Compaction 如何组成模型真正的工作上下文。'
date: 2026-06-15
...
**AGENTS.md、Skills 与 Compaction 如何组成模型真正的工作上下文。**

一种很常见但很危险的理解是：Codex 只是“把仓库内容塞进 prompt”。

这个说法太粗了，也解释不了 Codex 真正在做的事。Codex 的上下文不是一个越来越长的大字符串，而是一套运行时结构：项目规则有作用域，Skills 有触发条件，线程历史有账本，Compaction 有替换语义，初始上下文在基线变化后还能重新注入。
...
这不是一个一次性 prompt 能解决的问题。Codex 需要知道 `services/payments` 目录有本地规则；需要记住用户刚刚纠正过 staging feature flag；需要读日志、跑命令、调用工具，也可能要加载某个专门的 skill；最后还要在长对话快要超过上下文窗口时继续往下做。如果只是把所有内容暴力拼进 prompt，系统要么忘掉关键规则，要么被旧工具输出挤爆，要么在最需要继续时把当前任务压缩没了。

Codex 的 context engineering，就是为了让这种任务能活下来。

![The model context is a layered runtime](/assets/img/blog/codex-context-engineering-blog-zh/codex-context-stack.png)

## 1. 故事在模型开口之前就开始了

checkout bug 发生在 `services/payments`。在模型输出第一条命令之前，Codex 已经先组装了一条项目指令链。

这不是“顺手读几个文档”的约定，而是有明确作用域的机制。Codex 会在工作前读取 `AGENTS.md`。它先读取 Codex home 里的全局规则，然后从项目根目录一路走到当前工作目录。越靠近当前目录的指令越晚进入合并结果，因此可以覆盖更上层、更宽泛的规则。
...
![AGENTS.md builds the instruction spine](/assets/img/blog/codex-context-engineering-blog-zh/codex-agents-md-chain.png)

这里有两个细节很关键。

第一，Codex 在每个目录最多选择一个指令文件。它会优先看 override 文件，再看标准 `AGENTS.md`，最后才看配置里的 fallback 名称。这样每个目录的规则入口是清晰的，不会出现同一层多个文件互相打架。
...
最重要的结果是：项目记忆仍然是外部、可编辑、可审查的文件。新 session 或新 run 开始时，指令链会重新从文件构建，而不是藏在某种无法清理的模型记忆里。仓库规则是普通仓库资产，只是被 Codex 赋予了运行时语义。

## 2. Skills 不是常驻指令，而是等待触发的工作流

有了 `AGENTS.md` 之后，下一个诱惑是把所有有用流程都塞进 prompt：怎么 debug CI，怎么排查性能回归，怎么做 migration，怎么 review PR。短期看这很方便，长期看 prompt 会变成杂物间。
...
![Skills use progressive disclosure](/assets/img/blog/codex-context-engineering-blog-zh/codex-skills-progressive-disclosure.png)

这是一个很强的设计判断：专业知识应该先可发现，再按需加载。

源码路径也强化了这个判断。turn 构造阶段会先 build skills and plugins；用户输入里的显式 skill mention 会被收集；skill 描述也可以触发隐式调用。如果某个 skill 被选中，它的说明会作为 contextual user fragment 注入当前 turn。这说明 skill 不是全局政策，而是任务需要时才进入上下文的工作流。

预算约束同样关键。初始 skill 列表会被限制在模型上下文窗口的大约 2%，如果窗口未知则是 8,000 字符。这个限制迫使 skill description 写得足够短、足够准。skill 描述不是营销文案，而是路由元数据。

这也是为什么 `AGENTS.md` 和 Skills 不应该混用。

`AGENTS.md` 适合放长期项目规则：测试命令、代码风格、生成文件警告、本地约定、仓库特有的危险操作。Skills 适合放可复用流程：如何 debug GitHub Actions，如何做 release audit，如何安全迁移一个服务。把这两者混在一起，最后得到的不是更聪明的 agent，而是一堆永久指令。

## 3. Thread history 是运行时账本，不是聊天记录转储

当 Codex 开始工作后，上下文会不断变化。它读文件、跑测试、得到用户纠正、收到工具返回的 stack trace、改变下一步调查方向。这里面有些信息应该进入下一次 sampling，有些应该被归一化，有些最终应该被压缩。
...
![Thread history is a runtime ledger](/assets/img/blog/codex-context-engineering-blog-zh/codex-history-and-turn-loop.png)
...
`run_turn` 循环会继续采样和执行。如果模型请求工具调用，Codex 执行工具并记录结果；如果模型返回 assistant message，并且没有 pending work，turn 才能结束。在这些步骤之间，prompt 会从受管理的 history、当前 turn 状态、工具 specs、skill injections 和 context updates 重新构建出来。

这也是为什么 context engineering 和 tool runtime 不能分开看。一个工具结果如果没有进入受管理的 history，它只是一次观察；如果它以正确的位置进入 history，它就变成了 agent 的工作记忆。

## 4. Compaction 不是“写个更好的总结”，而是替换上下文窗口

长时间 agent 工作中，真正难的不是写总结，而是决定这个总结替换什么。
...
![Compaction replaces the window](/assets/img/blog/codex-context-engineering-blog-zh/codex-compaction-replacement-history.png)
...
Compaction 要做的是保留这些状态，同时丢掉高 token 噪音。
...
这个细节很小，但正好说明 compaction 必须属于 runtime。模型可以写总结，但 runtime 决定总结如何变成 history。
...
这里最关键的词是 “next sampling request”。上下文系统不是只准备第一条 prompt，而是在循环里不断准备每一次 prompt。

这也解释了为什么 `AGENTS.md`、Skills 和 Compaction 应该放在同一篇里讲。它们解决的是同一个 runtime 问题的不同失败模式：

![Context failure modes](/assets/img/blog/codex-context-engineering-blog-zh/codex-context-failure-modes.png)
...
这些机制单独看都不惊艳。放在一起，它们才组成模型真正的工作上下文。

## 6. 对 agent 工程的启发

关于 agent 设计，最常见的建议是“给模型正确上下文”。这句话没错，但太空泛。
...
当 Codex 修 checkout timeout 时，真正值得注意的不是它能不能编辑一个文件，而是模型能在一个由 runtime 组装、分层、限额、压缩、恢复的上下文窗口里持续工作。
