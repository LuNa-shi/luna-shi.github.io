...
这里先看不同插件提供的能力，怎样出现在那项失败测试任务中。等四条路径走完，再回来解释插件系统本身。
...
平时说到插件，我们很容易想到一种附加功能：浏览器扩展多做一件事，编辑器插件多提供一条命令，而软件本身基本不变。用这个印象理解 DeepSeek Harness，范围会窄很多。

先分清工具和插件。工具是模型可以请求的一项动作。插件则是参与运行系统的组件。它可以注册工具，也可以提供模型接入、补充上下文、保存任务记录、监听事件，或者改变某一步的执行方式。有些插件根本不会向模型提供工具。

插件系统给这些组件规定了一套进入和退出运行系统的方法。插件加载后，它提供的能力会在当前作用范围内生效，也可以使用其他插件提供的服务。插件卸载时，它注册过的内容应该一起消失。如果它依赖的服务发生变化，系统还要决定这个插件接下来怎样运行。

如果没有插件系统，主程序就要亲自创建模型连接、整理工具列表、打开 Session、安装权限检查，再把这些部分逐一接到 Agent Loop。每增加一种替换方式，核心运行系统里就要多一组连接逻辑。改成插件后，核心部分只需要协调当前有哪些能力，不必提前写死每一种具体实现。

DeepSeek Harness 使用 Cordis 管理这些插件。依赖和生命周期的具体规则会放到下一篇。本篇先记住一条简单流程：

```text
加载插件 -> 注册它提供的能力 -> 在任务中使用 -> 卸载时完整收回
```

这样再看 “everything is a plugin” 就准确多了。它没有把 Harness 的每个部分都变成模型可以调用的工具。它表达的是：运行系统中的主要职责，可以通过同一种扩展方式加入，并且保留替换的可能。

### 复杂度去了哪里

插件系统让团队可以更换模型服务，为不同任务开放不同工具，也能把文件操作和 Shell 移到隔离环境，而不用重写整条任务流程。这些部分之间的联系，随后要由运行系统的规则来保证。

以执行环境为例。文件工具已经指向远程沙箱，Shell 却仍在本机运行，Agent 面对的就是两套文件系统。它可能读取远程项目，又拿本地代码去测试。相似的接口解决不了这个问题。相关工具还要看见同一批文件，在同一个执行环境里工作。

卸载会暴露另一条规则。一个插件可能注册了工具、监听了事件、启动了后台任务，还依赖其他服务。删除插件名称很容易。运行系统还要移除监听、停止任务，并释放不该继续存在的资源。

依赖被替换时，问题会出现在系统运行途中。使用这项依赖的插件应该重新启动、暂时停用，还是继续拿着一个已经过期的对象？不同设计可以给出不同答案，但答案必须明确。插件加载顺序和碰巧残留的旧引用，不应该替系统作决定。

插件化也扩大了调试范围。一次测试失败，原因可能在模型判断、上下文组装、插件顺序、工具实现、权限决定或持久化后端。清楚的边界有助于定位问题，更多边界也意味着需要检查更多位置。

兼容性会直接进入架构。DeepSeek Harness 目前处于开发者预览阶段，官方明确提醒后续会有破坏兼容性的变化。当 Agent Loop 和任务记录都可以替换时，事件顺序、取消方式和记录格式不能只靠默认约定。具体实现可以更换，其他插件依赖的规则必须保持清楚。

安全方面的取舍更尖锐。增加一个只读搜索工具，获得的权限有限；能够修改工具策略或任务记录的插件，则靠近运行系统的信任边界。扩展位置要有足够能力完成工作，运行系统也要防止一个组件悄悄放宽另一个组件的权限。

所以，插件系统改变了复杂度所在的位置。原本写在主程序里的固定连接，变成了整套运行系统共同遵守的生命周期、依赖、兼容性、调试和安全规则。

Codex 与 Claude Code 同样需要准备上下文、执行工具、保留进度和限制操作。比较它们时，重点在于哪些边界被固定在产品内部，哪些边界允许用户配置或扩展。要作出更强的判断，还需要限定具体版本和工程问题。
